<?php
	class TwistBase {
    	public $key = "A3D2039E67A3A607";
    	private $product_id = "1";
    	private $product_base = "twist";
    	private $server_host = "https://codeixer.com/wp-json/el-api/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if(strpos(dirname($plugin_base_file),'wp-content/themes')!==FALSE){
				$this->isTheme=true;
			}
			$this->version=$this->getCurrentVersion();
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_twist_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=twist_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
					}
		...
		function __plugin_updateInfo(){
			if(function_exists("wp_remote_get")) {
				$response = wp_remote_get($this->server_host . "product/update/".$this->product_id);
		...
		final function checkUpdateInfo($false, $action, $arg) {
			if($this->isTheme){
				...
				if ( $arg->slug === plugin_basename($this->pluginFile) ) {
					$response =$this->__plugin_updateInfo();
					if ( !empty($response)) {
						return $response;
					}
		...
		private function _request( $relative_url, $data, &$error = '' ) {
			...
			if(function_exists('wp_remote_post')) {
				...
				if (is_wp_error($serverResponse)) {
					...
					if(!empty($serverResponse['body'])){
						return $this->processs_response($serverResponse['body']);
					}else{
						$response->msg    = "Response body empty";
						$response->status = false;
						$response->data = NULL;
						return $response;
					}
